...
class SendReportsAdminJob implements ShouldQueue
{
    ...
    {
        // send reports only to the users from tl list
        $tlListIds = TeamLeadsHelper::getTeamLeadsListIntervalIds();
        $approvals = Approval::LeftJoin('weeks', 'approvals.week_id', '=', 'weeks.id')
            ->where('approvals.status', false)
            ->whereIn('approvals.manager_id', $tlListIds)
            ->select(
                'approvals.week_id',
                'weeks.week_date_start',
                'weeks.week_date_end',
        ...
        foreach ($approval_unique as $item) { // unique period
            ...
            foreach ($managers_id as $manager_id) {
                ...
                foreach ($timereports as $key => $timereport) {
                    ...
                    if (empty($timereport['data'])) {
                        $reports->notReportedUsers[] = $timereport;
                    }
